...
module PaginationHelper
  def pagination_links_full(paginator, options = {})
    ...
    tag(:div, class: 'op-pagination') do
      content = tag(:nav,
  ...
  def pagination_option_links(paginator, pagination_options)
    ...
    tag(:div, option_links, class: 'op-pagination--options')
  end
  ...
  def pagination_entries(paginator, options)
    ...
    tag(:ul, class: 'op-pagination--items') do
      ...
      content << tag(:li, range, class: 'op-pagination--range', title: range)
  ...
  def pagination_settings(paginator, options)
    ...
    if links.size > 1
      ...
      tag(:ul, class: 'op-pagination--items') do
        tag(:li, label + ':', class: 'op-pagination--label', title: label) + links
      end
    end
  end
  ...
  def per_page_links(paginator, options)
    Setting.per_page_options_array.inject('') do |html, n|
      if n == paginator.per_page
        html + tag(:li, n, class: 'op-pagination--item_current')
      else
        link = link_to_content_update(n, options.merge(page: 1, per_page: n, class: 'button'))
        html + tag(:li, link.html_safe, class: 'op-pagination--item')
      end
    end.html_safe
  end
  ...
  class LinkRenderer < ::WillPaginate::ActionView::LinkRenderer
    ...
    def page_number(page)
      if page == current_page
        tag(:li, page, class: 'op-pagination--item_current')
      else
